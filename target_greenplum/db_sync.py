...
def column_type(schema_property):
    ...
    get_logger('target_greenplum').debug("schema_property: %s -> col_type: %s", schema_property, col_type)
...
def flatten_schema(d, parent_key=[], sep='__', level=0, max_level=0):
    ...
    for k, v in d['properties'].items():
        ...
        get_logger().info('====flatten_schema')
        get_logger().info(k)
        get_logger().info(v)
        get_logger().info('====flatten_schema')
        if 'type' in v.keys():
            ...
            if len(v.values()) > 0 and len(list(v.values())[0][0])>0:
...
class DbSync:
    def __init__(self, connection_config, stream_schema_message=None):
        ...
        self.logger = get_logger('target_greenplum')
