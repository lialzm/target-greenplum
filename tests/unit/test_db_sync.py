import target_greenplum
import unittest

import target_greenplum_
...
class TestUnit(unittest.TestCase):
    ...
    def test_config_validation(self):
        ...
        validator = target_greenplum.db_sync.validate_config
        empty_config = {}
        minimal_config = {
            'host':                     "dummy-value",
            'port':                     5432,
    ...
    def test_column_type_mapping(self):
        ...
        mapper = target_greenplum.db_sync.column_type
    ...
    def test_stream_name_to_dict(self):
        ...
        assert \
            target_greenplum.db_sync.stream_name_to_dict('my_table') == \
            {"catalog_name": None, "schema_name": None, "table_name": "my_table"}

        # Singer stream name format (Default '-' separator)
        assert \
            target_greenplum.db_sync.stream_name_to_dict('my_schema-my_table') == \
            {"catalog_name": None, "schema_name": "my_schema", "table_name": "my_table"}

        # Singer stream name format (Default '-' separator)
        assert \
            target_greenplum.db_sync.stream_name_to_dict('my_catalog-my_schema-my_table') == \
            {"catalog_name": "my_catalog", "schema_name": "my_schema", "table_name": "my_table"}

        # Redshift table format (Custom '.' separator)
        assert \
            target_greenplum.db_sync.stream_name_to_dict('my_table', separator='.') == \
            {"catalog_name": None, "schema_name": None, "table_name": "my_table"}

        # Redshift table format (Custom '.' separator)
        assert \
            target_greenplum.db_sync.stream_name_to_dict('my_schema.my_table', separator='.') == \
            {"catalog_name": None, "schema_name": "my_schema", "table_name": "my_table"}

        # Redshift table format (Custom '.' separator)
        assert \
            target_greenplum.db_sync.stream_name_to_dict('my_catalog.my_schema.my_table', separator='.') == \
            {"catalog_name": "my_catalog", "schema_name": "my_schema", "table_name": "my_table"}


    def test_flatten_schema(self):
        """Test flattening of SCHEMA messages"""
        flatten_schema = target_greenplum.db_sync.flatten_schema
    ...
    def test_flatten_record(self):
        ...
        flatten_record = target_greenplum.db_sync.flatten_record
